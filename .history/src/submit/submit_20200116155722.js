...
class Submit extends React.Component {
  ...
  // 获取数据
  getData = () => {
    const { dataSource,currentPage,data: d,} = this.state;
    window.Post("lifeCircle/lifeCircleFindByType",{  
      institutionTypeList:lifeState.info.tab,
      page:currentPage,
      pageSize:10,
    } ).then(({code,data,}) => {
      ...
      if (code === 0) {
        ...
        this.setState({
          // 要更新dataSource中的数据，每次都需要重新调用该方法
          dataSource: dataSource.cloneWithRows([...d,...data,]),
          data:[...d,...data,],
        });
      }
    });
